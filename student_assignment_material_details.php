...
// Get the $AAselect_id from the aafolder table based on $folder_id
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit'])) {
    $studentID = $_SESSION['user_id'];
    $folderID = intval($_GET['folder_id']); // Assuming you're passing folder_id through GET
    ...
    if (move_uploaded_file($fileTmpName, $destination)) {
        ...
        $insertQuery = "INSERT INTO assignment_submission (StudentID, AAfolderID, FileName, FilePath, FileType, FileSize, DateSubmission) VALUES (?, ?, ?, ?, ?, ?, NOW())";
        $stmt = $conn->prepare($insertQuery);
        $stmt->bind_param("iisssd", $studentID, $folderID, $fileName, $destination, $fileType, $fileSize);
...
$AssignmentfileQuery = "SELECT * FROM assignmentfile WHERE AAfolderID = $folder_id";
$AssignmentfileResult = $conn->query($AssignmentfileQuery);

$Assignmentfile = [];
if ($AssignmentfileResult->num_rows > 0) {
    while ($row = $AssignmentfileResult->fetch_assoc()) {
        $Assignmentfile[] = $row;
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_submit'])) {
    ...
    $deleteQuery = "DELETE FROM assignment_submission WHERE AssignsubID = ?";
    $stmt = $conn->prepare($deleteQuery);
    $stmt->bind_param("i", $deleteID);
    
    if ($stmt->execute()) {
        header("Location: {$_SERVER['PHP_SELF']}?folder_id={$folder_id}&student_id={$studentID}&assignteacher_id={$assignteacher_id}");
        exit();     
    } else {
        // Error in deletion
...
<div class="container">
    ...
    <div class="view-assignment-container">
        ...
        <div class="action-buttons">
                <button onclick="window.location='assignment_student_details.php?assignteacher_id=<?php echo $assignteacher_id; ?>&AAselect_id=<?php echo $AAselect_id; ?>&folder_id=<?php echo $folder_id; ?>';">Back</button>
        </div>
            ...
            <table>
                ...
                <tbody>
                    <tr>
                        ...
                        <td><?php echo $selectDueDate; ?></td>
                        <td><?php echo $selectFolderDate; ?></td>
                    </tr>
                </tbody>
            </table>
        </div>
        ...
        <div class="view-assignment-container">
                <table>
                    ...
                    <tbody>
                    <?php if (count($Assignmentfile) > 0): ?>
                            <?php foreach ($Assignmentfile as $index => $file): ?>
                        ...
                        <?php else: ?>
                            <tr>
                                <td colspan="4">No assignment file added yet.</td>
                            </tr>
                        <?php endif; ?>
                    </tbody>
                </table>
        ...
        <div class="clock" id="clock">
            <div class="clock-text">
                <span class="clock-label">DUE DATE:</span>
        ...
        <div class="submitted-files-container">
                ...
                <div class="file-list">
                    <?php foreach ($assignmentFiles as $file): ?>
                        <div class="file-item">
                            ...
                            <form class="delete-form" action="<?php echo $_SERVER['PHP_SELF']; ?>?folder_id=<?php echo $folder_id; ?>?student_id=<?php echo $studentID; ?>&assignteacher_id=<?php echo $assignteacher_id; ?>"" method="post">
                                <input type="hidden" name="delete_id" value="<?php echo $file['AssignsubID']; ?>">
            ...
            <?php else: ?>
                <p>No files submitted for this assignment.</p>
            <?php endif; ?>
        </div>

        <div class="submission-assignment-container">
            <form action="" method="POST" enctype="multipart/form-data">
                <div class="form-group">
                    <label for="file" style="margin-top:10px">Choose File:</label>
                    <input type="file" name="file" id="file">
                </div>
                <button type="submit" class="submit-btn" name="submit">Submit Assignment</button>
            </form>
        </div>
    </div>
</div>
...
<script>
    // Set the due date
    var dueDate = new Date("<?php echo $selectDueDate; ?>").getTime();
    ...
    function updateClock() {
        ...
        // If due date is not set or NaN, set dueDateText to appropriate message
        var dueDateText;
