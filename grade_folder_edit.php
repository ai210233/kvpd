...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($AAselect_id == 2 || $AAselect_id == 3) {
        // Get the maximum mark from the assessmentfolder
        $maxMarkQuery = "SELECT AssessmentMark FROM assessmentfolder WHERE AssessmentfolderID = $folder_id";
        $maxMarkResult = $conn->query($maxMarkQuery);
        if ($maxMarkResult->num_rows == 1) {
            $maxMarkRow = $maxMarkResult->fetch_assoc();
            $maxMark = intval($maxMarkRow['AssessmentMark']);
        } else {
            $maxMark = 100; // Default max mark if not found
        }
    } else {
        $maxMark = 100; // Default max mark for assignments
    }
    
    if ($mark < 0 || $mark > $maxMark) {
        echo "Error: Mark must be between 0 and $maxMark.";
    } else {
        if ($AAselect_id == 0 || $AAselect_id == 1) {
            // Check if the mark already exists for the given student and folder in the mark table
            $checkQuery = "SELECT * FROM mark WHERE StudentID = $student_id AND AAfolderID = $folder_id";
        } else {
            // Check if the mark already exists for the given student and folder in the mark table
            $checkQuery = "SELECT * FROM mark WHERE StudentID = $student_id AND AssessmentfolderID = $folder_id";
        }
        ...
        if ($checkResult->num_rows > 0) {
            ...
            if ($AAselect_id == 0 || $AAselect_id == 1) {
                $updateQuery = "UPDATE mark SET Marks = $mark WHERE StudentID = $student_id AND AAfolderID = $folder_id";
            } else {
                $updateQuery = "UPDATE mark SET Marks = $mark WHERE StudentID = $student_id AND AssessmentfolderID = $folder_id";
            }
            $conn->query($updateQuery);
        } else {
            // Insert new mark
            if ($AAselect_id == 0 || $AAselect_id == 1) {
                $insertQuery = "INSERT INTO mark (StudentID, AAfolderID, Marks) VALUES ($student_id, $folder_id, $mark)";
            } else {
                $insertQuery = "INSERT INTO mark (StudentID, AssessmentfolderID, Marks) VALUES ($student_id, $folder_id, $mark)";
            }
...
<div class="container">
    ...
    <div class="grade-folder-container">
        ...
        <div class="action-buttons">
            <button onclick="window.location='grade_folder_details.php?assignteacher_id=<?php echo $assignteacherID; ?>&AAselect_id=<?php echo $AAselect_id; ?>&folder_id=<?php echo $folder_id; ?>';">Back</button>
        </div>
        ...
        <form method="POST" action="">
            <div class="edit-grade-container">
                ...
                <input type="number" name="mark" id="mark" min="0" max="<?php echo $AAselect_id == 2 || $AAselect_id == 3 ? htmlspecialchars($material['AssessmentMark']) : '100'; ?>" required>
