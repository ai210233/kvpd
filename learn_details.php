...
$learningMaterialQuery = "SELECT LearnfolderID, FolderName, FolderDate, ShowFolder FROM learnfolder WHERE AssignteacherID = $assignteacherID";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['selectedRows'])) {
    ...
    $deleteQuery = "DELETE FROM learnfolder WHERE LearnfolderID IN ($placeholders)";
    ...
    <div class="edit-learn-container">
        ...
        <form method="POST" id="deleteForm">
            <div class="action-buttons">
                <button type="button" onclick="window.location='add_folder.php?assignteacher_id=<?php echo $assignteacherID; ?>';">Add New Folder</button>
                        ...
                        <?php if (count($learningMaterials) > 0): ?>
                            <?php foreach ($learningMaterials as $index => $material): ?>
                                <tr style="cursor: pointer;">
                                    ...
                                    <td>
                                        <input type="checkbox" name="selectedRows[]" value="<?php echo htmlspecialchars($material['LearnfolderID']); ?>" onclick="toggleDeleteButton()">
                                    </td>
                                    <td onclick="window.location='material_details.php?folder_id=<?php echo $material['LearnfolderID']; ?>&assignteacher_id=<?php echo $assignteacherID; ?>';"><?php echo $index + 1; ?></td>
                                    <td onclick="window.location='material_details.php?folder_id=<?php echo $material['LearnfolderID']; ?>&assignteacher_id=<?php echo $assignteacherID; ?>';"><?php echo htmlspecialchars($material['FolderName']); ?></td>
                                    <td onclick="window.location='material_details.php?folder_id=<?php echo $material['LearnfolderID']; ?>&assignteacher_id=<?php echo $assignteacherID; ?>';"><?php echo htmlspecialchars($material['FolderDate']); ?></td>
                                    <td onclick="window.location='material_details.php?folder_id=<?php echo $material['LearnfolderID']; ?>&assignteacher_id=<?php echo $assignteacherID; ?>';"><?php echo htmlspecialchars($material['ShowFolder'] == 0) ? 'Unlocked' : 'Locked'; ?></td>
                                </tr>
                            <?php endforeach; ?>
                        <?php else: ?>
                            <tr>
                                <td colspan="5">No folders added yet.</td>
                            </tr>
                        <?php endif; ?>
                    </tbody>
                </table>
