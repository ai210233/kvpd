...
<div class="container">
    ...
    <div class="assignment-folder-container">
        ...
        <div class="action-buttons">
            <button onclick="window.location='assignment_material_details.php?assignteacher_id=<?php echo $assignteacherID; ?>&AAselect_id=<?php echo $AAselect_id; ?>&folder_id=<?php echo $folder_id; ?>';">Back</button>
        </div>
            ...
            <tbody>
                ...
                if ($studentResult->num_rows > 0) {
                    ...
                    while ($studentRow = $studentResult->fetch_assoc()) {
                        ...
                        $submissionQuery = "SELECT FileName, DateSubmission, FilePath FROM assignment_submission WHERE AAfolderID = $folder_id AND StudentID = " . $studentRow['StudentID'];
                        ...
                        if ($submissionResult->num_rows > 0) {
                            ...
                            while ($submissionData = $submissionResult->fetch_assoc()) {
                                ...
                                $dueDate = strtotime($editFolder['DueDate']);
                                $status = ($submissionDate <= $dueDate) ? "On-time" : "Late";
                                echo "<td>$status</td>";
                                echo "</tr><tr>";
                            }
                        } else {
...
<script>
    // Set the due date
    var dueDate = new Date("<?php echo $editFolder['DueDate']; ?>").getTime();
    ...
    function updateClock() {
        ...
        // If distance is negative, set dueDateText to LATE SUBMISSION
        // If due date is not set, set dueDateText to NO DUE DATE
        var dueDateText;
        if (distance < 0) {
            dueDateText = "LATE SUBMISSION";
        } else if (dueDate === null || isNaN(dueDate)) {
            ...
            dueDateText = "DUE DATE: " + days + " Days " + hours + " Hours " + minutes + " Minutes " + seconds + " Seconds ";
        }

        // Display the remaining time or appropriate message
        var clockElement = document.getElementById("clock");
        clockElement.innerHTML = dueDateText;
    }
